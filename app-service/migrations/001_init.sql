-- Migration 001: initial Postgres schema (ticket #19 lineage).
-- See PRD.md §8 and ARCHITECTURE.md §3.5 for design rationale.
-- Applied by `pnpm db:migrate` / applySchema via migrations/ journal.
--
-- Idempotent (CREATE TABLE/INDEX IF NOT EXISTS) so existing databases that
-- already applied the former root schema.sql can run this once and record
-- it in schema_migrations without failing. No down-migration.
