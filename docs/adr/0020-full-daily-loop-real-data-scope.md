...
`app-service/src/server.test.ts`'s new "full daily loop sequence" test does exactly that: creates a real learner, runs several real sessions through the actual `POST /sessions` → `GET .../scenario` → `POST .../observations` → `GET .../debrief` → `GET .../callback` cycle, asserts real (non-fixture-shaped) responses throughout, asserts the real least-recently-used scenario rotation (#21) actually produces more than one scenario across the sequence (not a static value), and confirms the daily cap (#24) still rejects the next session after the real sequence. This is a genuine, real, run-repeatedly-stable proof of AC #2's substance, minus the literal "3-5 calendar days on a physical device" framing, which — like Converse itself — needs a human.
